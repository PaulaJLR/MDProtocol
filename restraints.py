...
class PositionRestraints:
    ...
    def apply(self, minim:bool=False):
        ...
        restraint = CustomExternalForce(f'{self.config.name}*periodicdistance(x, y, z, x0, y0, z0)^2')
        restraint.addGlobalParameter(self.config.name, weight)
        ...
        self.equilibration.system.addForce(restraint)
        self.restraint = restraint
        self.restraint.setName(self.config.name)
    ...
    def remove(self):
        ...
        for index in range(self.equilibration.system.getNumForces()):
            ...
            if force.getName() == self.restraint.getName():
                self.equilibration.system.removeForce(index)
                self.equilibration.simulation.context.reinitialize(preserveState=True)
                break # must break!
    ...
    def get_weight_list(self):
        ...
        stage2_numsteps = int(np.round( end_time/dt - start_time/dt ))
        ...
        stage3 = [0.0] * int( np.round(npt_restr_time/dt - end_time/dt) )
